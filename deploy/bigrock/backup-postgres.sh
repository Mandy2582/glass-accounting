...
SUPABASE_DIR="${SUPABASE_DIR:-/opt/arjun-supabase}"
...
if [ -f "$SUPABASE_DIR/docker-compose.yml" ]; then
  docker compose --project-directory "$SUPABASE_DIR" exec -T db \
    pg_dump --username "$DB_USER" --format custom "$DB_NAME" > "$FILE"
else
